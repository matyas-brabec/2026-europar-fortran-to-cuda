! CDW correctness test
! Calls the kernel once on a deterministic input and dumps the interior
! of W2 to stdout (one value per line) for cross-variant comparison.
!
...
program cdw_test
    ...
    call CDW(W2, U, V, W, dxmin, dymin, dzmin, NX, NY, NZ)

    ! Dump interior (ghost-free) of W2, one value per line, full precision
    do k = 2, NZ+1
        do j = 2, NY+1
            do i = 2, NX+1
                write(*,'(g0.17)') W2(i,j,k)
            end do
        end do
    end do

    deallocate(U, V, W, W2)
end program cdw_test
