! CDV correctness test
! Calls the kernel once on a deterministic input and dumps the interior
! of V2 to stdout (one value per line) for cross-variant comparison.
!
...
program cdv_test
    ...
    ! Dump interior (ghost-free) of V2, one value per line, full precision
    do k = 2, NZ+1
        do j = 2, NY+1
            do i = 2, NX+1
                write(*,'(g0.17)') V2(i,j,k)
            end do
        end do
    end do

    deallocate(U, V, W, V2)
end program cdv_test
